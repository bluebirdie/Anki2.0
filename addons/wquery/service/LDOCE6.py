...
class Ldoce6(MdxService):
    ...
    @export(u'DefinitionEN', 5)
    def fld_definition(self):
        html = self.get_html()
        m = re.findall(r'<span class="def">(.*?)</span def>', html)
        if m:
            if len(m) == 1:
               return m[0]
            else:
               return '1.'+m[0] + '<br>' +'2.'+ m[1]
        return ''   
        
    @export(u'DefinitionCN', 6)
    def fld_definitionCN(self):
        html = self.get_html()
        m = re.findall(r'<span class="def_c">(.*?)</span def_c>', html)
        if m:
            if len(m) == 1:
               return m[0]
            else:
